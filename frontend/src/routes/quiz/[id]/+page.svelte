...
>>>>>>> a6f256911bd91da0b979b46a8d9484a08d4142a9
    ...
    try {
      ...
      // Check if quiz has ended (friendly response from backend)
      ...
      // Handle 410 or other errors gracefully
...
<<<<<<< HEAD
  ...
  function handleFullscreenChange() {
    ...
    if (!isCurrentlyFullscreen && !quizTerminated && !submitting) {
  ...
  async function recordCheatingAttempt(reason) {
    ...
    if (cheatingWarnings === 1) {
      ...
      warningMessage = `❌ QUIZ TERMINATED: ${reason}. Your quiz has been automatically submitted due to multiple cheating attempts. Your teacher has been notified.`;
      showWarningModal = true;
      quizTerminated = true;
      
      // Notify teacher
      try {
        await api.request('/report-cheating', {
          method: 'POST',
          body: {
            quiz_id: quizId,
            student_id: $user.id,
...
>>>>>>> a6f256911bd91da0b979b46a8d9484a08d4142a9
  ...
  function nextQuestion() {
    // Don't lock when manually clicking next
    recordQuestionTime();
...
=======
      goto(`/results/${quizId}?score=${result.score}&total=${result.total_questions}`);
>>>>>>> a6f256911bd91da0b979b46a8d9484a08d4142a9
    } catch (err) {
