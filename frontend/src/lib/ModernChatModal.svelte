<script>
  ...
  let theme = 'dark'; // 'dark', 'light', 'blue'
...
<!-- Modern Gen-Z Chat Interface -->
<div class="fixed inset-0 bg-gradient-to-br from-purple-900/95 via-blue-900/95 to-pink-900/95 backdrop-blur-sm flex items-center justify-center z-[50] p-2 sm:p-4 animate-fadeIn">
  <div class="bg-gradient-to-br {currentTheme.bg} rounded-3xl shadow-2xl w-full max-w-6xl h-[95vh] flex flex-col border-2 {currentTheme.border} overflow-hidden transition-all duration-500">
    
    <!-- Header with Gradient -->
    <div class="relative flex items-center justify-between p-4 sm:p-6 bg-gradient-to-r {currentTheme.accent} text-white">
      <div class="flex items-center space-x-3">
        <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm animate-pulse">
          <span class="text-2xl">💬</span>
        </div>
        <div>
          <h2 class="text-xl sm:text-2xl font-bold tracking-tight">Vibe Chat</h2>
          <p class="text-xs text-white/80">Stay connected, stay lit 🔥</p>
        </div>
      </div>
      <div class="flex items-center space-x-2">
        <button
          on:click={cycleTheme}
          class="w-10 h-10 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition-all backdrop-blur-sm"
          title="Change theme"
        >
          {#if theme === 'dark'}
            <Moon size={20} />
        ...
        <button on:click={closeChat} class="w-10 h-10 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition-all hover:rotate-90 duration-300 backdrop-blur-sm">
          <X size={20} />
        </button>
      </div>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- Sidebar - Room List -->
      <div class="w-full sm:w-80 border-r {currentTheme.border} {currentTheme.card} flex flex-col backdrop-blur-sm transition-all duration-500">
        {#if currentUser?.role === 'admin' || currentUser?.role === 'teacher'}
          <div class="p-3 border-b border-gray-700/50">
            <button
              on:click={() => showCreateRoom = true}
              class="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-3 rounded-2xl hover:from-purple-600 hover:to-pink-600 transition-all font-semibold shadow-lg hover:shadow-purple-500/50 transform hover:scale-105 duration-200"
            >
              ✨ Create New Vibe
            </button>
          </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          {#if loading}
            <div class="p-4 text-center">
              ...
              <p class="text-gray-400 mt-2 text-sm">Loading vibes...</p>
            </div>
          {:else if rooms.length === 0}
            <div class="p-6 text-center">
              <div class="text-6xl mb-3">😴</div>
              <p class="text-gray-400 text-sm">No chat rooms yet</p>
              <p class="text-gray-500 text-xs mt-1">Create one to start vibing!</p>
            </div>
          {:else}
            {#each rooms as room}
              <button
                class="w-full text-left p-4 border-b border-gray-800/50 hover:bg-gradient-to-r hover:from-purple-900/30 hover:to-pink-900/30 transition-all duration-200 {selectedRoom?.id === room.id ? 'bg-gradient-to-r from-purple-900/50 to-pink-900/50 border-l-4 border-l-purple-500' : ''}"
                on:click={() => selectRoom(room)}
              >
                <div class="flex items-center justify-between">
                  <div class="flex-1 min-w-0">
                    <div class="font-semibold text-white text-sm truncate">{room.name}</div>
                    <div class="text-xs text-gray-400 mt-1">{getRoomTypeDisplay(room.room_type)}</div>
                    {#if room.department && room.level}
                      <div class="text-xs text-purple-400 mt-0.5">{room.department} - {room.level}</div>
                    {/if}
                  </div>
