...
def now():
    ...
    return datetime.now(RWANDA_TZ).replace(tzinfo=None)
...
try:
    ...
    print("Optional dependencies not installed. File upload features disabled.")

# Import Enhanced AI Question Parser
try:
    from enhanced_ai_question_parser import EnhancedAIQuestionParser, parse_text_file_bytes
    AI_PARSER_ENABLED = True
except ImportError:
    AI_PARSER_ENABLED = False
    print("Enhanced AI Question Parser not available.")
...
# Admin routes will be defined below in the file after get_current_user is defined
...
class User(Base):
    ...
    department = Column(String(100))  # For students - increased length
    level = Column(String(50))  # For students - increased length
...
# Note: get_current_user is defined later in the file
...
def login(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    print(f"Login attempt for username: '{username}' (length: {len(username)})")
...
def get_questions(
    ...
    print(f"📝 Questions fetched for {current_user.username}:")
    print(f"   Department: {department}, Level: {level}")
    print(f"   Total questions: {len(questions)}")
    for q in questions[:3]:
        print(f"   - {q.question_text[:50]}... (Type: {q.question_type})")
...
def get_quizzes(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for quiz in quizzes:
        ...
        if quiz.countdown_started_at:
            elapsed = (now() - quiz.countdown_started_at).total_seconds()
...
def get_quiz_questions(quiz_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Check if user already attempted this quiz FIRST (before time check)
    attempt = db.query(QuizAttempt).filter(
    ...
    if attempt and current_user.role == "student":
        # Return friendly message for already attempted quiz
        return {
    ...
    if quiz.countdown_started_at:
        ...
        elapsed_time = (now() - quiz.countdown_started_at).total_seconds()
        
        if elapsed_time > total_quiz_time:
            # Return friendly JSON response instead of HTTP error
    ...
    # Shuffle questions if enabled and user is a student
    if quiz.shuffle_questions and current_user.role == "student":
        import random
        # Create a deterministic shuffle based on user ID and quiz ID
...
def broadcast_quiz(quiz_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    FIXED VERSION: Properly broadcasts quiz to ALL students in the target department/level
    """
    if current_user.role != "teacher":
        raise HTTPException(status_code=403, detail="Teacher or Admin access required")
    ...
    current_time = now()
    quiz.is_active = True
    quiz.scheduled_time = current_time
    quiz.countdown_started_at = current_time
    
    print(f"🕐 Setting countdown_started_at to: {current_time}")
    print(f"🕐 Quiz duration: {quiz.duration_minutes} minutes")
    ...
    print(f"🕐 After commit, countdown_started_at is: {quiz.countdown_started_at}")
    ...
    print(f"Students found matching {quiz.department} - {quiz.level}: {len(students)}\n")
    ...
    for s in all_students:
        match_status = "✅ MATCH" if (s.department == quiz.department and s.level == quiz.level) else "❌ NO MATCH"
        print(f"   {s.username:15} | {s.department:30} | {s.level:15} | {match_status}")
    print()
...
def get_quiz_status(quiz_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get quiz status including expiry information - for teacher UI to show rebroadcast button"""
    quiz = db.query(Quiz).filter(Quiz.id == quiz_id).first()
    if not quiz:
        raise HTTPException(status_code=404, detail="Quiz not found")
...
def upload_schedule_file(
    ...
    try:
        ...
        db_schedule = Schedule(
            title=title or f"Weekly Timetable - {now().strftime('%Y-%m-%d')}",
...
def get_lessons(
    ...
    if current_user.role == "admin":
        ...
        return query.order_by(Lesson.department, Lesson.level, Lesson.title).all()
    elif current_user.role == "teacher":
...
def update_lesson(lesson_id: int, lesson: LessonCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Teacher access required")
    ...
    if lesson.code != db_lesson.code:
        existing = db.query(Lesson).filter(Lesson.code == lesson.code, Lesson.id != lesson_id).first()
        if existing:
            raise HTTPException(status_code=400, detail="Lesson code already exists")
    
    db_lesson.title = lesson.title
    db_lesson.code = lesson.code
    db_lesson.description = lesson.description
    db_lesson.department = lesson.department
    db_lesson.level = lesson.level
    db_lesson.classification = lesson.classification
...
def delete_lesson(lesson_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    questions_count = db.query(Question).filter(Question.lesson_id == lesson_id).count()
    if questions_count > 0:
        raise HTTPException(status_code=400, detail=f"Cannot delete lesson with {questions_count} questions. Delete questions first or deactivate lesson.")
    
    # Check if lesson has teacher assignments
    assignments_count = db.query(TeacherLesson).filter(TeacherLesson.lesson_id == lesson_id).count()
    if assignments_count > 0:
        raise HTTPException(status_code=400, detail=f"Cannot delete lesson with {assignments_count} teacher assignments. Remove assignments first.")
...
@app.put("/lessons/{lesson_id}/activate")
def activate_lesson(lesson_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Teacher access required")
    ...
    lesson.is_active = True
    db.commit()
    return {"message": "Lesson activated"}

# Teacher assignment endpoints
@app.get("/teachers")
def get_teachers(
    department: Optional[str] = None,
    include_class_teachers: Optional[bool] = None,
    current_user: User = Depends(get_current_user), 
    db: Session = Depends(get_db)
):
    ...
    if include_class_teachers is not None:
        query = query.filter(User.is_class_teacher == include_class_teachers)
    ...
    for teacher in teachers:
        ...
        if department and teacher.departments:
            if department not in teacher.departments:
                continue
        
        # Get assigned lessons count
        lessons_count = db.query(TeacherLesson).filter(TeacherLesson.teacher_id == teacher.id).count()
        
        # Get class teacher assignments
        class_assignments = db.query(ClassTeacher).filter(ClassTeacher.teacher_id == teacher.id).all()
        
        result.append({
            "id": teacher.id,
            "username": teacher.username,
            "full_name": teacher.full_name,
            "departments": teacher.departments or [],
...
def get_teacher_details(teacher_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Teacher access required")
    ...
    lessons = db.query(Lesson, TeacherLesson).join(TeacherLesson).filter(
        TeacherLesson.teacher_id == teacher_id
    ).all()
    
    # Get class assignments
    class_assignments = db.query(ClassTeacher).filter(ClassTeacher.teacher_id == teacher_id).all()
    ...
    return {
        ...
        "assigned_lessons": [{
            ...
            "level": lesson.level,
            "assignment_id": assignment.id
        } for lesson, assignment in lessons],
        "class_assignments": [{
            "id": ca.id,
            "department": ca.department,
            "level": ca.level,
...
def delete_teacher(teacher_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Teacher access required")
    ...
    questions_count = db.query(Question).filter(Question.created_by == teacher_id).count()
    quizzes_count = db.query(Quiz).filter(Quiz.created_by == teacher_id).count()
    
    if questions_count > 0 or quizzes_count > 0:
        raise HTTPException(
            status_code=400, 
            detail=f"Cannot delete teacher with {questions_count} questions and {quizzes_count} quizzes. Reassign or delete them first."
        )
    
    # Delete teacher assignments
    db.query(TeacherLesson).filter(TeacherLesson.teacher_id == teacher_id).delete()
...
def get_teacher_lessons(teacher_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Allow admin or the teacher themselves to access
    if current_user.role not in ["admin", "teacher"]:
        raise HTTPException(status_code=403, detail="Teacher or admin access required")
...
def get_student_report(quiz_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    story.append(Spacer(1, 20))
    story.append(Paragraph(f"<i>Generated: {now().strftime('%Y-%m-%d %H:%M:%S')} UTC</i>", styles['Italic']))
...
# Enhanced AI Question Parser Endpoint
@app.post("/questions/upload-text-ai")
def upload_questions_ai(
    file: UploadFile = File(...),
    department: str = Form(...),
    level: str = Form(...),
    lesson_id: int = Form(...),
    ...
    """Upload and parse questions using Enhanced AI Parser"""
    if current_user.role != "teacher":
        raise HTTPException(status_code=403, detail="Teacher access required")
    
    if not AI_PARSER_ENABLED:
        raise HTTPException(status_code=501, detail="AI Question Parser not available")
    ...
    if not lesson:
        raise HTTPException(status_code=400, detail=f"Invalid lesson selection for {department} - {level}")
    
    try:
        # Read file content
        content = file.file.read()
        
        # Parse using Enhanced AI Parser
        result = parse_text_file_bytes(content, department, level)
        ...
        for i, question_data in enumerate(questions, 1):
            try:
                # Validate question
                parser = EnhancedAIQuestionParser()
                is_valid, error_msg = parser.validate_question(question_data)
                ...
                # Format for database
                formatted_question = parser.format_for_database(question_data)
                
                # Create database question
                db_question = Question(
                    question_text=formatted_question['question_text'],
                    question_type=formatted_question['question_type'],
                    options=formatted_question.get('options', []),
                    correct_answer=formatted_question['correct_answer'],
                    points=formatted_question.get('points', 1),
        ...
        # Commit valid questions
        if created_questions:
            db.commit()
        
        # Prepare response
        total_parsed = result.get('total', 0)
        total_saved = len(created_questions)
        type_counts = result.get('type_counts', {})
        
        # Create summary message
        type_summary = ', '.join([f"{count} {qtype}" for qtype, count in type_counts.items()])
        message = f"✅ Uploaded {total_saved} questions ({type_summary})"
        
        if errors:
            message += f". {len(errors)} questions had errors."
...
def upload_text_questions(
    ...
    if current_user.role == "teacher":
        ...
        if not assignment:
            raise HTTPException(
                status_code=403,
                detail="You can only upload questions for lessons assigned to you"
            )
    ...
    if not lesson:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid lesson for {department} - {level}"
        )
    
    try:
        # Read and parse file
        content = file.file.read()
        result = parse_text_file_content(content, department, level)
        ...
        for q_data in result['questions']:
            try:
                ...
                errors.append(f"Error saving question: {str(e)}")
...
def upload_questions(file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "teacher":
        raise HTTPException(status_code=403, detail="Teacher or Admin access required")
...
def parse_text_file_content(file_content: bytes, department: str = "", level: str = ""):
    """Parse text file and return structured data"""
    try:
        ...
        questions = parse_text_questions(content, department, level)
...
def parse_text_questions(content: str, department: str = "", level: str = ""):
    ...
    for line in lines:
        ...
        if not line or line.startswith('=') or line.startswith('Module:') or \
           line.startswith('Department:') or line.startswith('Level:') or \
           line.startswith('FORMAT NOTES') or line.startswith('END OF'):
            continue
        ...
        if question_match:
            ...
            question_text = current_question['text'].lower()
            if question_text.endswith(' true') or question_text.endswith(' false'):
                # Extract the answer and clean the question
                if question_text.endswith(' true'):
                    current_question['answer'] = 'True'
                    current_question['text'] = current_question['text'][:-5].strip()
                else:
                    current_question['answer'] = 'False'
                    current_question['text'] = current_question['text'][:-6].strip()
                
                # Add as True/False question immediately
                questions.append({
                    'question_text': current_question['text'],
        ...
        if answer_match and current_question:
            ...
            if len(answer) == 1 and answer.upper() in 'ABCD':
                # Letter answer - map to option text
                answer_index = ord(answer.upper()) - ord('A')
                if 0 <= answer_index < len(current_options):
                    current_question['answer'] = current_options[answer_index]
            else:
                # Direct text answer
                current_question['answer'] = answer
            
            # If we have options, it's MCQ, otherwise short answer
            if current_options:
                questions.append({
                    'question_text': current_question['text'],
                    'question_type': 'mcq',
...
def upload_students_file(file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role != "teacher":
        raise HTTPException(status_code=403, detail="Teacher access required")
...
# Admin: Upload students via multiple formats
try:
    from backend.student_pdf_parser import parse_student_pdf
    from backend.student_import import parse_student_excel
except Exception:
    try:
        from student_pdf_parser import parse_student_pdf  # type: ignore
        from student_import import parse_student_excel  # type: ignore
    except Exception:
        parse_student_pdf = None
        parse_student_excel = None
...
def debug_quizzes(db: Session = Depends(get_db)):
    """Debug endpoint to see all quizzes in the system"""
    ...
    return {
        ...
        "quizzes": [{
            ...
            "countdown_started_at": q.countdown_started_at,
...
def debug_students(db: Session = Depends(get_db)):
    """Debug endpoint to see all students in the system"""
    all_students = db.query(User).filter(User.role == "student").all()
    return {
...
def admin_upload_students(
    students_data: StudentUpload,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Upload students in bulk - ADMIN ONLY (NO MORE TEACHER ACCESS REQUIRED ERROR!)"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    for student_data in students_data.students:
        try:
            if not all(k in student_data for k in ["username", "full_name", "department", "level"]):
                errors.append(f"Missing required fields for {student_data.get('full_name', 'unknown')}")
                continue
            ...
            errors.append(f"Error processing {student_data.get('full_name', 'unknown')}: {str(e)}")
    
    db.commit()
    
    return {
        "success": True,
        "message": f"Students uploaded: {created_count} created, {updated_count} updated",
...
def admin_generate_student_credentials(
    department: str,
    level: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Generate PDF with student credentials - ADMIN ONLY"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    
    # Get students for this department and level
    students = db.query(User).filter(
        User.role == "student",
    ...
    try:
        ...
        story.append(Paragraph("🎓 TVET QUIZ SYSTEM", title_style))
        story.append(Paragraph("Student Login Credentials", styles['Heading2']))
        story.append(Spacer(1, 20))
        
        # Class info
        info_style = styles['Normal']
        story.append(Paragraph(f"<b>Department:</b> {department}", info_style))
        story.append(Paragraph(f"<b>Level:</b> {level}", info_style))
        story.append(Paragraph(f"<b>Total Students:</b> {len(students)}", info_style))
        story.append(Paragraph(f"<b>Generated:</b> {now().strftime('%Y-%m-%d %H:%M')}", info_style))
        story.append(Spacer(1, 20))
        
        # Credentials table
        data = [['#', 'Student Name', 'Username', 'Password']]
        ...
        story.append(Paragraph("<b>📝 Login Instructions:</b>", styles['Heading3']))
        story.append(Paragraph("1. Go to the quiz system URL provided by your teacher", styles['Normal']))
        story.append(Paragraph("2. Enter your username and password exactly as shown above", styles['Normal']))
        story.append(Paragraph("3. Click 'Sign In' to access your student dashboard", styles['Normal']))
        story.append(Paragraph("4. You can change your password after first login", styles['Normal']))
        story.append(Spacer(1, 15))
        story.append(Paragraph("<b>⚠️ Important Notes:</b>", styles['Heading3']))
        story.append(Paragraph("• Keep your credentials confidential", styles['Normal']))
        story.append(Paragraph("• Do not share your password with others", styles['Normal']))
        story.append(Paragraph("• Contact your teacher if you forget your password", styles['Normal']))
...
@app.post("/admin/upload-students-excel")  # Alias for frontend compatibility
def admin_upload_students_file(
    file: UploadFile = File(...),
    department: str = Form(...),
    level: str = Form(...),
    current_user: User = Depends(get_current_user),
    ...
    print(f"📤 Upload request: file={file.filename}, dept={department}, level={level}, user={current_user.username}")
    ...
    if file_ext not in ['pdf', 'xls', 'xlsx']:
        raise HTTPException(
            status_code=400,
            detail="❌ Only Excel files (.xlsx, .xls) and PDF files (.pdf) are supported"
        )
    ...
    try:
        ...
        if file_ext in ['xls', 'xlsx']:
            ...
            for i, row in enumerate(sheet.iter_rows(min_row=1, max_row=20, values_only=True), 1):
                if row[0]:
                    ...
                    if 's/n' in cell_value or 'sn' in cell_value or 'no' in cell_value:
                        start_row = i + 1
                        break
            ...
            for row in sheet.iter_rows(min_row=start_row, values_only=True):
                try:
                    if len(row) > 1 and row[1]:
                        ...
                        if name and name.lower() not in ['names', 'name', 'none', ''] and len(name) > 2:
            ...
            print(f"📄 PDF text (first 500 chars): {text[:500]}")
            ...
            for line in lines:
                ...
                skip_words = ['logo', 'header', 'footer', 'page', 'date', 'signature']
                if any(word in line.lower() for word in skip_words):
                    continue
                if line.startswith(('_', '-', '=', '*')) and len(line) < 10:
                    continue
                ...
                # Pattern 2: Just text with 2+ words
                elif len(line.split()) >= 2:
                    name = line
                
                if name:
                    name = re.sub(r'\s+', ' ', name).strip()
                    # Accept any name with letters
                    if len(name) > 2 and not name.isdigit() and any(c.isalpha() for c in name):
                        students.append(name)
                        print(f"✅ Found: {name}")
            
            print(f"📊 Total extracted: {len(students)}")
        
        if not students:
            raise HTTPException(
                status_code=400,
                detail=f"❌ No student names found in {file_ext.upper()} file. Please check the file format."
            )
        ...
        for full_name in students:
            try:
                ...
                if existing:
                    # Update existing student with ALL fields
                    existing.full_name = full_name
                    existing.department = department
                    existing.level = level
                    existing.password_hash = password_hash
                    existing.role = 'student'  # Ensure role is set
                    db.flush()  # Flush to ensure update is applied
                    updated_count += 1
                    print(f"✅ Updated: {username} -> {department} - {level}")
                else:
                    new_student = User(
                        username=username,
                    ...
                    db.add(new_student)
                    db.flush()  # Flush to ensure insert is applied
                    created_count += 1
                    print(f"✅ Created: {username} -> {department} - {level}")
        ...
        # Return response with proper statistics
        return {
            'success': True,
            'message': f'✅ Successfully imported {len(credentials)} students from {file.filename}',
...
def admin_clear_all_students(
    ...
    """Delete all students from the system - ADMIN ONLY"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    try:
        ...
        # 2. Delete quiz attempts and student answers
        attempt_ids = [a.id for a in db.query(QuizAttempt).filter(QuizAttempt.user_id.in_(student_ids)).all()]
        if attempt_ids:
        ...
        db.query(QuizResults).filter(QuizResults.user_id.in_(student_ids)).delete(synchronize_session=False)
        
        # 4. Now delete all students
        ...
        return {
            "success": True,
            "message": f"Successfully deleted {count} students and all related records",
            "count": count
        }
...
def admin_register_teacher(
    ...
    return {
        ...
        "teacher": {
            ...
            "departments": new_teacher.departments,
            "is_class_teacher": new_teacher.is_class_teacher,
...
def export_session_plan_pdf(plan_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        story.append(Paragraph(f"Generated on {now().strftime('%Y-%m-%d %H:%M')} | RTB Session Plan Template", footer_style))
...
def export_scheme_of_work_pdf(scheme_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        story.append(Spacer(1, 30))
        footer_style = ParagraphStyle('Footer', parent=styles['Normal'], fontSize=9, alignment=1, textColor=colors.grey)
        story.append(Paragraph(f"Generated on {now().strftime('%Y-%m-%d %H:%M')} | RTB Scheme of Work Template", footer_style))
...
# BULK UPLOAD - STEP 1 
from bulk_question_upload import handle_bulk_upload 
from template_generator import generate_question_template
from enhanced_ai_question_parser import EnhancedAIQuestionParser, parse_text_file_bytes 
 
@app.post("/questions/upload-bulk") 
async def upload_questions_bulk( 
    ...
    if current_user.role != "teacher": 
        raise HTTPException(status_code=403, detail="Teacher access required") 
    return await handle_bulk_upload(file, department, level, lesson_id) 


@app.post("/questions/upload-text-ai")
async def upload_questions_text_ai(
    file: UploadFile = File(...),
    department: str = Form(...),
    level: str = Form(...),
    lesson_id: int = Form(...),
    ...
    if current_user.role == "teacher":
        ...
        if not assignment:
            raise HTTPException(status_code=403, detail="Lesson not assigned to you")
    ...
    if not lesson:
        raise HTTPException(status_code=400, detail=f"Invalid lesson")
    ...
    try:
        ...
        for i, q_data in enumerate(questions, 1):
            try:
                if not q_data.get('question_text'):
                    errors.append(f"Q{i}: Missing text")
                    continue
        ...
        return {
            "success": True,
            "message": f"✅ Uploaded {len(created_questions)} questions ({type_summary})",
...
@app.get("/questions/template/excel") 
async def download_question_template(): 
    from fastapi.responses import StreamingResponse 
    import io 
    template_bytes = generate_question_template() 
    return StreamingResponse( 
