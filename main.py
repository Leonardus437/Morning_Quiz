...
from sqlalchemy import create_engine, Column, Integer, String, Text, Boolean, DateTime, JSON, ForeignKey, Float, text
...
def parse_advanced_question(text):
    """Enhanced parser for real-world documents with flexible formatting"""
    import re
    ...
    if re.search(r'\b(true|false)\b', text, re.IGNORECASE) and re.search(r'\?', text):
        ...
        if not result['answer']:
            result['answer'] = 'true'
    elif re.search(r'select all|choose all|multiple correct|tick all', text, re.IGNORECASE):
        result['type'] = 'multiple_select'
    elif re.search(r'fill.{0,15}blank|complete.{0,15}sentence|fill.{0,15}gap', text, re.IGNORECASE):
        result['type'] = 'fill_blanks'
    elif re.search(r'match|pair|connect|correspond', text, re.IGNORECASE):
        result['type'] = 'matching_pairs'
    elif re.search(r'order|arrange|sequence|sort|rank', text, re.IGNORECASE):
        result['type'] = 'drag_drop_ordering'
    elif re.search(r'rate|scale|1.{0,5}10|rating|rank from', text, re.IGNORECASE):
        result['type'] = 'linear_scale'
    elif re.search(r'write.{0,10}code|program|function|algorithm|implement', text, re.IGNORECASE):
        result['type'] = 'code_writing'
    elif re.search(r'sql|query|database|select\s+\*|insert\s+into', text, re.IGNORECASE):
        result['type'] = 'sql_query'
    elif re.search(r'dropdown|select from|choose from dropdown', text, re.IGNORECASE):
        result['type'] = 'dropdown_select'
    elif re.search(r'short answer|brief|briefly|one.{0,10}sentence', text, re.IGNORECASE):
        result['type'] = 'short_answer'
    elif re.search(r'essay|discuss|elaborate|explain.{0,10}detail|describe.{0,10}detail', text, re.IGNORECASE):
        result['type'] = 'essay'
    else:
        result['type'] = 'multiple_choice'
    
    # Extract options using multiple patterns (A), A., a), a., etc.
    option_patterns = [
        r'([A-Z])\)\s*([^A-Z\)\n]+)',  # A) option
        r'([A-Z])\.\s*([^A-Z\.\n]+)',  # A. option
        r'([a-z])\)\s*([^a-z\)\n]+)',  # a) option
    ...
    # If no answer found but has options, try to extract from option markers
    if not result['answer'] and result['options']:
        # Look for marked correct answer (e.g., "*A)" or "A) [correct]")
        marked_pattern = r'[\*✓✔]\s*([A-Za-z])[\)\.]'
        marked = re.search(marked_pattern, result['text'])
        if marked:
            result['answer'] = marked.group(1).upper()
...
def startup_event():
    # Create all tables including Assignment
    Base.metadata.create_all(bind=engine)
    
    # Migrate existing assignments table to add missing columns
    db = SessionLocal()
    try:
        # Add missing columns to assignments table if they don't exist
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS assignment_type VARCHAR(50) DEFAULT 'document'"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS allow_late_submission BOOLEAN DEFAULT false"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS is_published BOOLEAN DEFAULT false"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS school_id INTEGER REFERENCES schools(id)"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS trade_id INTEGER REFERENCES trades(id)"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS level VARCHAR(10)"))
        db.execute(text("ALTER TABLE assignments ADD COLUMN IF NOT EXISTS file_path VARCHAR(500)"))
        
        # Add missing columns to assignment_submissions table if they don't exist
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS file_path VARCHAR(500)"))
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS text_content TEXT"))
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS link_url VARCHAR(500)"))
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS score FLOAT"))
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS feedback TEXT"))
        db.execute(text("ALTER TABLE assignment_submissions ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'submitted'"))
        
        # Create assignment_comments table for chat
        db.execute(text("""
        ...
        db.commit()
        print("✅ Assignment table migration completed")
    except Exception as e:
        print(f"⚠️ Assignment migration error (may be safe to ignore): {e}")
        db.rollback()
